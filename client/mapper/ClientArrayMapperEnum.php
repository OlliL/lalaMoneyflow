<?php

namespace rest\client\mapper;

class ClientArrayMapperEnum {
	const CAPITALSOURCE_ARRAY_TYPE = 'CapitalsourceClientArray';
	const CONTRACTPARTNER_ARRAY_TYPE = 'ContractpartnerClientArray';
	const MONEYFLOW_ARRAY_TYPE = 'MoneyflowClientArray';
	const PREDEFMONEYFLOW_ARRAY_TYPE = 'PreDefMoneyflowClientArray';
	const USER_ARRAY_TYPE = 'UserClientArray';
	const SESSION_ARRAY_TYPE = 'SessionClientArray';
}

?>