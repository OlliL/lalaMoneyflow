...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_text` AS select `a`.`id` AS `id`,`a`.`text` AS `text`,`a`.`type` AS `type`,`b`.`userid` AS `userid` from (`text` `a` join `settings` `b`) where ((`a`.`languageid` = `b`.`value`) and (`b`.`name` = _latin1'displayed_language')) */;
