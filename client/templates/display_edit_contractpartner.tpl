...
{$HEADER}
		...
		<form action="{$ENV_INDEX_PHP}" method="POST">
			...
			<table border=0>
				...
				<tr>
					<td class="contrastbgcolor"><input class="contrastbgcolor" type="text" name="all_data[name]"      value="{$ALL_DATA.name}"                        {if $ALL_DATA.name_error              == 1}style="color:red"{/if}></td>
					...
					{section name=POSTINGACCOUNT loop=$POSTINGACCOUNT_VALUES}
						<option value="{$POSTINGACCOUNT_VALUES[POSTINGACCOUNT].postingaccountid}" {if $POSTINGACCOUNT_VALUES[POSTINGACCOUNT].postingaccountid == $ALL_DATA.mpa_postingaccountid}selected{/if} > {$POSTINGACCOUNT_VALUES[POSTINGACCOUNT].name}</option>
					{/section}
					</select></td>
				</tr>
			</table>
